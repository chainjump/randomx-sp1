...
A real Groth16 proof of Monero block 3,727,837 has been generated by the
Succinct prover network, verified locally with SP1, and successfully passed an
Ethereum-mainnet `eth_call` verifier simulation. The request, proof identity,
cost, and verification evidence are recorded in
...
On 2026-07-28, both the tagged image and its immutable digest reproduced the
retained ELF byte for byte: 295,352 bytes, SHA-256
`ac3eff37cbae4583f57cdbc193cca776a80672c77a63c09eb507dc35d154c317`,
and a successful `cmp`. The Docker-built ELF independently derived the same
program vkey used by the completed network proof, and that proof again passed
the Ethereum-mainnet verifier simulation when the Docker-built ELF was
supplied to the client. No duplicate paid proof was needed because the build
output is the exact proved binary. Commands, image digests, timings, and
verification results are in `evidence/reproducible-build.md`.
...
release. Its existing proof and vkey remain tied to the compatible SP1 circuit
and verification system; a future incompatible SP1 circuit may require a new
build and proof.
...
Current evidence is under `evidence/`. In particular,
`evidence/reproducible-build.md` records the container build and
`evidence/network-proof/README.md` records the paid proof and EVM simulation.
The SP1-specific unsafe-code, syscall, ELF-layout, dependency, and provenance
review is recorded in `evidence/sp1-program-safety-review.md`. Rejected
candidates and earlier artifacts remain recoverable from local Git history.
There is no configured Git remote, so preserving or backing up this
repository's `.git` directory is required for that recovery path.
