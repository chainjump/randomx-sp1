...
pub struct Instr {
    ...
    /// RandomX's precomputed 64-bit reciprocal for `IMUL_RCP`.
    ///
    /// Zero means that this instruction is not `IMUL_RCP`, or that its
    /// divisor is zero/a power of two and the RandomX instruction is a NOP.
    pub reciprocal: u64,
...
pub fn decode_instruction(bytes: i64, i: i32, register_usage: &mut [i32; MAX_REG]) -> Instr {
    ...
    if op < Opcode::IMUL_RCP as i64 {
        // RandomX defines the divisor as the instruction's unsigned imm32.
        // Convert through u32 before widening so 0x8000_0000 stays a power of
        // two instead of being sign-extended to 0xffff_ffff_8000_0000.
        ...
        #[cfg(feature = "precompute-reciprocal")]
        if !is_zero_or_power_of_2(divisor) {
            instr.reciprocal = randomx_reciprocal(divisor);
        }
        return instr;
    }
...
mod reciprocal_tests {
    ...
    fn encoded_imul_rcp(divisor: u32) -> i64 {
        (((divisor as u64) << 32) | 0x4c) as i64
    }

    #[test]
    fn vm_reciprocal_is_computed_during_decode() {
        for divisor in [3_u32, 7, 0x7fff_ffff, 0x8000_0001, 0xffff_ffff] {
            let mut usage = [-1; MAX_REG];
            let instr = decode_instruction(encoded_imul_rcp(divisor), 0, &mut usage);
            assert_eq!(instr.op, Opcode::IMUL_RCP);
            assert_eq!(
                instr.reciprocal,
                randomx_reciprocal(divisor as u64),
                "divisor {divisor:#x}"
            );
        }
    }
    ...
    #[test]
    fn vm_power_of_two_reciprocal_remains_a_nop() {
        for divisor in [0_u32, 1, 2, 4, 1 << 30] {
            let mut usage = [-1; MAX_REG];
            let instr = decode_instruction(encoded_imul_rcp(divisor), 0, &mut usage);
            assert_eq!(instr.op, Opcode::IMUL_RCP);
            assert_eq!(instr.reciprocal, 0, "divisor {divisor:#x}");
        }
    }
    ...
    fn vm_unsigned_high_bit_power_of_two_matches_reference_nop() {
        ...
        assert_eq!(instr.op, Opcode::IMUL_RCP);
        assert_eq!(instr.reciprocal, 0);
        assert_eq!(usage, [-1; MAX_REG]);
    }
}
