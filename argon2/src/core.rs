...
const RANDOMX_SALT: &[u8; 8] = b"RandomX\x03";
const RANDOMX_LANES: u32 = 1;
const RANDOMX_HASH_LENGTH: u32 = 0;
const RANDOMX_ITERATIONS: u32 = 3;
...
pub fn initialize_randomx(key: &[u8]) -> Box<[Block]> {
    ...
    let mut initialized = unsafe { blocks.assume_init() };
    let initialized_ptr = initialized.as_mut_ptr();
...
fn initial_hash(key: &[u8]) -> [u8; common::PREHASH_SEED_LENGTH] {
    let mut state = Params::new()
        .hash_length(common::PREHASH_DIGEST_LENGTH)
        .to_state();
    for value in [
        RANDOMX_LANES,
        RANDOMX_HASH_LENGTH,
    ...
    state.update(key);
    state.update(&(RANDOMX_SALT.len() as u32).to_le_bytes());
    state.update(RANDOMX_SALT);
    state.update(&0u32.to_le_bytes()); // secret length
    state.update(&0u32.to_le_bytes()); // associated-data length

    let mut out = [0u8; common::PREHASH_SEED_LENGTH];
    out[..common::PREHASH_DIGEST_LENGTH].copy_from_slice(state.finalize().as_bytes());
    out
}
...
mod randomx_specialization_tests {
    ...
    fn reference_index(pass: u32, slice: u32, index: u32, pseudo_rand: u32) -> u32 {
    ...
    #[test]
    fn randomx_reference_formula_matches_optimized_boundaries() {
            ...
            for slice in 0..common::SYNC_POINTS {
                ...
                for index in indices {
                    for pseudo_rand in pseudo_random_values {
                        let reference = reference_index(pass, slice, index, pseudo_rand);
