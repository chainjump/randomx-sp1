...
pub fn hash_aes_1rx4(input: &[u64]) -> [m128i; 4] {
    ...
    assert_eq!(
        input.len() % 8,
        0,
        "AES input must contain complete 64-byte blocks"
    );
    ...
    for chunk in input.chunks_exact(8) {
        let in0 = m128i::from_u64(chunk[1], chunk[0]);
        let in1 = m128i::from_u64(chunk[3], chunk[2]);
        let in2 = m128i::from_u64(chunk[5], chunk[4]);
        let in3 = m128i::from_u64(chunk[7], chunk[6]);
...
pub fn fill_aes_1rx4_u64(input: &[m128i; 4], into: &mut Vec<u64>) -> [m128i; 4] {
    assert_eq!(
        into.len() % 8,
        0,
        "AES output must contain complete 64-byte blocks"
    );
    ...
    for chunk in into.chunks_exact_mut(8) {
...
fn fill_aes_1rx4_m128i(input: &[m128i; 4], into: &mut Vec<m128i>) -> [m128i; 4] {
    ...
    let mut state2 = input[2];
    let mut state3 = input[3];
    for chunk in into.chunks_exact_mut(4) {
        state0 = state0.aesdec(key0);
        state1 = state1.aesenc(key1);
        state2 = state2.aesdec(key2);
        state3 = state3.aesenc(key3);
